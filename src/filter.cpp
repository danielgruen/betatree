...
// ObjectPrototype
  ...
  ObjectPrototype::ObjectPrototype(const int keys[], const string names[], const Type types[], int n, string prefix) : intSortVkey(-1), doubleSortVkey(-1), sortedBy(-1) 
  {
   for (int i=0; i<n; i++)
   {
    switch (types[i]) 
...
// Object
  ...
  double Object::doublePropertyValue(int vkey) const
  {
   if(vkey<0 || vkey>=doubleProperty.size()) {
     cerr << "You have asked for an invalid double property " << vkey << endl;
     cerr << "I have " << doubleProperty.size() << " properties" << endl;
     cerr << "my prototype is " << *prototype << endl;
  ...
  {
   assert(vkey>=0);
   assert(vkey<stringProperty.size());
  ...
  {
   if(!file) {
    cerr << "file not open! exiting" << endl; 
    cerr << "this was a call of ObjectCollection constructor with zero ifstream";
    for(int i=0; i<n; i++) cerr << keys[i] << " " << names[i] << " " << types[i] << endl;
    exit(1); 
   }
...
#else
   for (multimap<string, Column *>::iterator iter = colmap.begin(); (iter != colmap.end() && (columns.size()==0 || intcolumns.size()+doublecolumns.size()+stringcolumns.size()<columns.size())); iter++) {
#endif
  ...
  {
   if(!file) {
    cerr << "ObjectCollection::appendFile file not open! exiting" << endl; exit(1); 
   }
   while(1)
   {
  ...
  void ObjectCollection::extendCollection(ObjectCollection &other, string fromcolumn, string tocolumn)
  {
  	if(other.size() != this->size()) {
	 cerr << "cannot extend catalog by columns of different row count; ignoring" << endl;
	 return;
	...
	int cother = other.prototype->doubleVkey(fromcolumn);
	if(cother>=0) { // double
	  int c = createDoublePropertyIfNecessary(tocolumn);
#pragma omp parallel for
	...
	cother = other.prototype->intVkey(fromcolumn);
	if(cother>=0) { // int
	  int c = createIntPropertyIfNecessary(tocolumn);
#pragma omp parallel for
	  for(int i=0; i<objects.size(); i++) 
	  {
	    objects[i]->setIntProperty(c,other.objects[i]->intPropertyValue(cother));
...
#pragma omp critical
  ...
  {
   appendCollectionDeep(&cc);
  }
   
  void ObjectCollection::appendCollectionDeep(ObjectCollection *cc)
  {
   for(int i=0; i<cc->size(); i++) {
    Object *o = new Object(*(cc->objects[i]));
  ...
  {
   //cerr << "# filtering for " << f.name << endl;
   int key = prototype->intVkey(f.name);
   if(key<0) 
   {
     //cerr << "# it's a double" << endl;
     key=prototype->doubleVkey(f.name);
     if(key<0) {
       cerr << "filtering for invalid double key " << f.name << endl; 
     }
     return filterDouble(key, f.minVal, f.maxVal); 
   }
   //cerr << "# it's an int" << endl;
  ...
  ObjectCollection* ObjectCollection::filter(vector<Filter> f) const
  {
   ...
   for(int i=0; i<f.size(); i++)
   {
    ...
    cerr << "filtering by unknown column " << f[i].minVal << "<=" << f[i].name << "<=" << f[i].maxVal << ", aborting!" << endl; 
    
    cerr << "my columns are " << *prototype << endl; 
   ...
   vector<Object*>::const_iterator itend = objects.end();
   
   /*
   vector<int>::const_iterator vd = find(doubleVkeys.begin(), doubleVkeys.end(), prototype->sortedByDouble());
   if(vd != doubleVkeys.end()) {
    // objects are sorted and we filter by the sort key, limit iterator range
    Object *omin = new Object(prototype);
    omin->setDoubleProperty(*vd,doubleF[vd-doubleVkeys.begin()].minVal);
    Object *omax = new Object(prototype);
    omax->setDoubleProperty(*vd,doubleF[vd-doubleVkeys.begin()].maxVal);
    it = lower_bound(it, itend, omin, ObjectPrototype::sortObjects);
    itend = upper_bound(it, itend, omax, ObjectPrototype::sortObjects);
    delete omin; delete omax;
   }
   */
...
//      	cerr << "phi=" << phi << ", g1=" << g1 << ", g2=" << g2 << ", gt=" << objects[i]->doubleProperty[gtkey] << ", gx=" << objects[i]->doubleProperty[gxkey] << endl;
  ...
  double ObjectCollection::sum(string colname) const
  {
    ...
    if(ckey>=0) {
      return this->sum(ckey);
    }
    ckey=prototype->intVkey(colname);
    if(ckey>=0) {
      return this->sum(ckey, true);
    }
    cerr << "# you are asking me to sum a non-existing column " << colname << " and I am rightfully refusing to" << endl;
    return -1.e99;
  }
  double ObjectCollection::sum(int doubleVkey, bool is_integer) const
  {
    
    double s=0.;
    if(!is_integer) {
      for(vector<Object*>::const_iterator it = objects.begin(); it != objects.end(); ++it) {
	    s += (*it)->doubleProperty[doubleVkey];
      }
    } else {
      for(vector<Object*>::const_iterator it = objects.begin(); it != objects.end(); ++it) {
	    s += (*it)->intProperty[doubleVkey];
      }
    }
    return s;
  }
  double ObjectCollection::sumSq(string colname) const
  {
    ...
    if(ckey>=0) {
      for(vector<Object*>::const_iterator it = objects.begin(); it != objects.end(); ++it) {
	    s += pow((*it)->doubleProperty[ckey],2);
      }
      return s;
    }
    else if((ckey=prototype->intVkey(colname)>=0)) {
      for(vector<Object*>::const_iterator it = objects.begin(); it != objects.end(); ++it) {
	    s += double(pow((*it)->intProperty[ckey],2));
      }
      return s;
    }
    cerr << "# you are asking me to sum a non-existing column " << colname << " and I am rightfully refusing to" << endl;
...
#pragma omp parallel for reduction(+ : s)
  ...
  double ObjectCollection::average(int doubleVkey, bool is_integer) const
  {
    double sumt=sum(doubleVkey, is_integer);
    return sumt/size();
  }
...
#pragma omp critical
  ...
  double ObjectCollection::minimum(string colname)
  {
    int dckey=prototype->doubleVkey(colname);
    int ickey=prototype->intVkey(colname);
    ...
    if(dckey>=0)
    {
     double dmin = objects[0]->doubleProperty[dckey];
     int imin=0;
     
     for(int i=1; i<size(); i++)
     {
       if(objects[i]->doubleProperty[dckey]<dmin)
       {
	dmin = objects[i]->doubleProperty[dckey];
	imin = i;
       }
     }
    ...
    if(ickey>=0)
    {
     int dmin = objects[0]->intProperty[ickey];
     int imin=0;
     
     for(int i=1; i<size(); i++)
     {
       if(objects[i]->intProperty[ickey]<dmin)
       {
	dmin = objects[i]->intProperty[ickey];
	imin = i;
       }
     }
    ...
    cerr << "ERROR: cannot determine minimum of unknown double or int column " << colname << endl;
    return 0./0.;
    
  }


  double ObjectCollection::maximum(string colname)
  {
    int dckey=prototype->doubleVkey(colname);
    int ickey=prototype->intVkey(colname);
    ...
    cerr << "ERROR: cannot determine maximum of unknown double or int column " << colname << endl;
    return 0./0.;
    
  }
  
  double ObjectCollection::sigma68(string colname)
  {
     return (percentile(colname,1.-(1.-0.6827)/2.)-percentile(colname,(1.-0.6827)/2.))/2.;
  }
...
#pragma omp parallel for
  ...
  void ObjectCollection::transformColumn(string colname, int (*f)(void)) {
    int ckey=prototype->doubleVkey(colname);
    if(ckey<0) {
     ckey=prototype->intVkey(colname);
     if(ckey<0) {
...
#pragma omp parallel for
    for(int iit=0; iit<objects.size(); iit++) {
	  objects[iit]->doubleProperty[ckey]=(*f)();
    }    
  }
   

  void ObjectCollection::transformColumn(string colname, int (*f)(int)) {
     int ckey=prototype->intVkey(colname);
     if(ckey<0) {
      cerr << "ERROR: cannot transform unknown int column " << colname << endl;
      return;
     } 
...
#pragma omp parallel for
  ...
  int ObjectCollection::transformColumnNew(string newcolname, double (*f)(void)) {
...
#pragma omp parallel for
  ...
  int ObjectCollection::transformColumnNew(string newcolname, int (*f)(int), string arg) {
    int ckey=createIntPropertyIfNecessary(newcolname, -1, false);
    int a1=prototype->intVkey(arg);
    if(a1<0) {
      cerr << "ERROR: cannot use unknown int column for transformation" << endl;
...
#pragma omp parallel for
    for(int iit=0; iit<objects.size(); iit++) {
     Object **it= &(objects[iit]);
      (*it)->intProperty[ckey] = (*f)((*it)->intProperty[a1]);
    }
...
#pragma omp parallel for
  ...
  int ObjectCollection::transformColumnNew(string newcolname, int (*f)(double, double), string arg1, string arg2) {
    int ckey=createIntPropertyIfNecessary(newcolname, -1, false);
    int a1=prototype->doubleVkey(arg1);
    int a2=prototype->doubleVkey(arg2);
    if(a1<0 || a2<0) {
...
#pragma omp parallel for
    for(int iit=0; iit<objects.size(); iit++) {
     Object **it= &(objects[iit]);
      (*it)->intProperty[ckey] = (*f)((*it)->doubleProperty[a1], (*it)->doubleProperty[a2]);
    }
    return ckey;
  }
...
#pragma omp parallel for
  ...
  int ObjectCollection::transformColumnNew(string newcolname, double (*f)(double, double, int), string arg1, string arg2, string arg3) {
    int ckey=createDoublePropertyIfNecessary(newcolname, -1, false);
    int a1=prototype->doubleVkey(arg1);
    int a2=prototype->doubleVkey(arg2);
    int a3=prototype->intVkey(arg3);
    if(a1<0 || a2<0 || a3<0) {
      cerr << "ERROR: cannot use unknown double column for transformation" << endl;
      if(a1<0) cerr << "missing: " << arg1 << endl;
      if(a2<0) cerr << "missing: " << arg2 << endl;
...
void ObjectCollection::sort(string sortcolumn, bool force)
{
    if(prototype->setSortKey(sortcolumn) || force)
    {
      //cerr << "# sorting by " << sortcolumn << endl;
      std::sort(objects.begin(), objects.end(), ObjectPrototype::sortObjects);
    } else {
      //cerr << "# already sorted by " << sortcolumn << endl; 
    }
}
...
ObjectCollection* ObjectCollection::tree(const vector<string> &criteria, const vector<double> &minwidth, int nmin, string target, double badflag)
// build a tree such that in each leaf there are at least nmin members (but more than 2*nmin if no significant difference in target)
{
  ...
  vector<double> min(criteria.size());
  vector<double> max(criteria.size());
  ...
  for(int i=0; i<criteria.size(); i++)
  {
      ...
      min[i] = minimum(criteria[i]);
      max[i] = FilterFunctions::max(badflag,maximum(criteria[i]));
  }
  ...
  growTree(criteria, minwidth, min, max, nmin, target, leaves, badflag);
  delete []binnames; delete []binkeys; delete []bintypes; delete leavesPrototype;
  return leaves;
}

void ObjectCollection::growTree(const vector<string> &criteria, const vector<double> &minwidth, vector<double> min, vector<double> max, int nmin, string target, ObjectCollection *leaves, double badflag)
{
  cerr << "growing tree ";
  for(int i=0; i<min.size(); i++)
    cerr << criteria[i] << "=" << min[i] << "..." << max[i] << " ";
  cerr << endl;
  
  // stop if below size limit
  if(size()<2*nmin) {
     cerr << "cell with n=" << size() << " not populated enough to split, saving" << endl; 
  ...
  // max snr cut
  int cmax=-1; double snrmax=0.; double medmax=0.;
  ...
  for(int i=0; i<criteria.size(); i++)
  {
     ...
     // get median
     sort(criteria[i],true);
     double med=(objects[int((size()-1)/2)]->doublePropertyValue(criteria[i])+
                objects[int((size()+1)/2)]->doublePropertyValue(criteria[i]))/2.;
 
     if(med==badflag) // don't split if median is already in border
     {
       continue;
     }
     
     // split in two
     ObjectCollection *tsmall = filter(Filter(criteria[i],min[i],med));
     ObjectCollection *tlarge = filter(Filter(criteria[i],med,max[i]));
     ...
     double snr    = fabs(asmall-alarge)/sqrt(ssmall*ssmall+slarge*slarge);
     if(snr>snrmax) {
       cmax=i; snrmax=snr; medmax=med; 
  ...
  if(snrmax>2.) {
    cerr << "found a significant difference when splitting by crit " << cmax << "; branching" << endl;
    double dmax=max[cmax];
    max[cmax]=medmax;
    tsmallmax->growTree(criteria, minwidth, min, max, nmin, target, leaves, badflag);
    max[cmax]=dmax; min[cmax]=medmax;
    tlargemax->growTree(criteria, minwidth, min, max, nmin, target, leaves, badflag);
  } else {
...
void ObjectCollection::treeAverage(ObjectCollection *tree, vector<string> criteria, string property, string nobjstring)
{
   int meankey=-1;
   if(property!="")
     meankey=tree->createDoublePropertyIfNecessary(property+"_mean");
   int nkey=tree->createIntPropertyIfNecessary(nobjstring);
...
#pragma omp parallel for
   for(int i=0; i<tree->size(); i++) {
     ...
     for(int j=0; j<criteria.size(); j++) {
       vf.push_back(Filter(
         criteria[j],
         (*tree)[i]->doublePropertyValue(criteria[j]+"_min"),
         (*tree)[i]->doublePropertyValue(criteria[j]+"_max")
       ));
       //cerr << " : " << criteria[j] << " " << (*tree)[i]->doublePropertyValue(criteria[j]+"_min") 
            //<< " " << (*tree)[i]->doublePropertyValue(criteria[j]+"_max") << flush;
     }
     ObjectCollection *tmp = filter(vf);
     //cerr << " with " << tmp->size() << " objects" << endl;
     double a = -1;
     if(property!="") a=tmp->average(property);
     int s = tmp->size();
     delete tmp;
     #pragma omp critical
     {
     if(property!="") (*tree)[i]->setDoubleProperty(meankey,a);
     (*tree)[i]->setIntProperty(nkey,s);
     }
   }
}

    
void ObjectCollection::applyBinAverage(ObjectCollection *bintable, string column, vector<string> binning_columns, string binaveragecolumn_name, string leafcol)
{
  int tbkey=bintable->prototype->doubleVkey(column);
  int obkey=createDoublePropertyIfNecessary(binaveragecolumn_name);
  int lkey=-1;
  if(leafcol!="") lkey=createIntPropertyIfNecessary(leafcol);
...
#pragma omp parallel for
  ...
  for(int i=0; i<objects.size(); i++) {
	...
	if(  (*bintable)[b]->doublePropertyValue(binning_column_minvkeys[s]) > objects[i]->doublePropertyValue(object_column_vkeys[s]) ||
	     (*bintable)[b]->doublePropertyValue(binning_column_maxvkeys[s]) < objects[i]->doublePropertyValue(object_column_vkeys[s])   ) {     
	     //cerr << "no!" << endl;
	     goto outofbin;
	}
	//cerr << "yes!" << endl;
    ...
    if (inbin<0)
    {
     ...
     for(int s = 0; s < binning_columns.size(); s++) {
       cerr << binning_columns[s] << "=" << objects[i]->doublePropertyValue(object_column_vkeys[s]) << endl;
     }
    ...
    objects[i]->setDoubleProperty(obkey,(*bintable)[inbin]->doublePropertyValue(tbkey));
    if(lkey>=0) objects[i]->setIntProperty(lkey, inbin);
  }
}
  ...
  void ObjectCollection::writeToFITS(string filename, vector<string> doubleColumns, vector<string> intColumns, string extension) const
  {
    ...
    vector<string> colName(doubleColumns.size()+intColumns.size(),"");
    vector<string> colForm(doubleColumns.size()+intColumns.size(),"");
    vector<string> colUnit(doubleColumns.size()+intColumns.size(),"");
    
    for(int i=0; i<doubleColumns.size(); i++)
    {
      colName[i]=doubleColumns[i];
      colForm[i]="D";
    }
    for(int i=doubleColumns.size(); i<doubleColumns.size()+intColumns.size(); i++)
    {
      colName[i]=intColumns[i-doubleColumns.size()];
      colForm[i]="J";
    }
    
    Table* newTable = pFits->addTable(extension,size(),colName,colForm,colUnit);
    
    for(int i=0; i<doubleColumns.size(); i++) 
    {
      vector<double> c(size());
      int k = prototype->doubleVkey(colName[i]);
      if(k>=0) {
        for(int j=0; j<size(); j++)
        {
          c[j] = objects[j]->doublePropertyValue(k);
        }
      } else {
        k = prototype->intVkey(colName[i]);
        cerr << colName[i] << " " << k << endl;
        assert(k>=0);
        for(int j=0; j<size(); j++)
        {
          c[j] = objects[j]->intPropertyValue(k);
        }
      }
      newTable->column(colName[i]).write(c,1);
    }    
    for(int i=doubleColumns.size(); i<doubleColumns.size()+intColumns.size(); i++) 
    {
      vector<int> c(size());
      ...
      if(k>=0) {
        for(int j=0; j<size(); j++)
        {
          c[j] = int(objects[j]->doublePropertyValue(k));
        }
      } else {
        k = prototype->intVkey(colName[i]);
        assert(k>=0);
...
double ObjectCollection::tophatDensity(string cx, string cy, double x, double y, double r, bool sortagain) { // 2d top-hat kernel density with radius r at x,y
    int ix = prototype->doubleVkey(cx);
    int iy = prototype->doubleVkey(cy);
    if(ix<0 || iy<0) {
     cerr << "x or y column not found" << endl; return 0.; 
    ...
    if(sortagain) sort(cx);
    
    // look for range closer in x than r
    ...
    while(imin_upper-imin_lower>1)
    {
      imin=(imin_upper+imin_lower)/2;
      if(objects[imin]->doubleProperty[ix]<x-r)
	imin_lower=imin;
      else
	imin_upper=imin;
    }
    ...
    //cout << imin_lower << " " << objects[imin_lower]->doubleProperty[ix] << " " << x << " " << r << endl;
    
    int n=0;
    for (int i=imin_lower; i<imax; i++)
    {
     if(objects[i]->doubleProperty[ix]<x-r)
       continue;
     if(objects[i]->doubleProperty[ix]>x+r)
       break;
      
     if(fabs(objects[i]->doubleProperty[iy]-y)<r)
     {
      if(FilterFunctions::pow2(objects[i]->doubleProperty[iy]-y)+FilterFunctions::pow2(objects[i]->doubleProperty[ix]-x)<r2)
	n++;
     }
    }
    
    return double(n)/(M_PI*r2);
}

double ObjectCollection::CramerVonMises(string column, ObjectCollection *other, string othercolumn) { 
// 2-sided CvM statistic
    int mykey    = prototype->doubleVkey(column);
    int otherkey = other->prototype->doubleVkey(othercolumn);
    if(mykey<0 || otherkey<0) {
     cerr << "CvM column not found" << endl; return 0.; 
    }
    ...
    other->sort(othercolumn,true);
    //cerr << "now sorted" << endl;
        
    const int keys[]  = {1, 2};
    const string names[] = {"value", "myorother"};
    const Type types[] = {doubleType, integerType};
    ObjectPrototype *p = new ObjectPrototype(keys,names,types,2);
    ObjectCollection *cvm = new ObjectCollection(p);
    
    //cerr << "now prepared list" << endl;
    for(int i=0; i<size(); i++)
    {
      Object *o = new Object(cvm->prototype);
      //cerr << i << " " << o->doubleProperty.size() << " " << o->intProperty.size() << endl;
      o->doubleProperty[0] = objects[i]->doubleProperty[mykey];
      o->intProperty[0]    = 1; // mine
      cvm->appendObject(o);
    }
    for(int i=0; i<other->size(); i++)
    {
      Object *o = new Object(cvm->prototype);
      o->doubleProperty[0] = other->objects[i]->doubleProperty[otherkey];
      o->intProperty[0]    = 2; // other
      cvm->appendObject(o);
    }
    //cerr << "now filled list" << endl;
    
    cvm->sort("value");
    //cout << "concatenated, sorted list" << endl;
    //cout << (*cvm) << endl;
    ...
    {
      if(cvm->objects[i]->intProperty[0]==1) { // mine
        U += 1./double(M)*double(i-myrank)*double(i-myrank);
        myrank++;
      }
      else { // other
        //cout << "other is adding " << 1./double(N)*double(i-otherrank)*double(i-otherrank) << endl;
        U += 1./double(N)*double(i-otherrank)*double(i-otherrank);
        otherrank++;
      }
    }
    ...
    return U/double(N+M)-double(4*M*N-1)/double(6*(M+N));
}
