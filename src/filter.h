...
class ObjectPrototype {
  ...
  public:
    ...
    ObjectPrototype(): intSortVkey(-1), doubleSortVkey(-1), sortedBy(-1) { }; // empty prototype
    ObjectPrototype(ObjectPrototype *pp): intSortVkey(-1), doubleSortVkey(-1), sortedBy(-1), intPropertyName(pp->intPropertyName), doublePropertyName(pp->doublePropertyName), stringPropertyName(pp->stringPropertyName), intPropertyKey(pp->intPropertyKey), doublePropertyKey(pp->doublePropertyKey), stringPropertyKey(pp->stringPropertyKey) { };
    ...
    bool setSortKey(string columnname) // return 0 if already set, 1 otherwise
    {
      //cerr << "# setting sort key " << columnname << " " << sortedBy << " " << intSortVkey << " " << doubleSortVkey << endl;
      int di = intVkey(columnname);
      int dd = doubleVkey(columnname);
      //cerr << "di=" << di << " dd=" << dd << endl;
      
      if(sortedBy==0 && di==intSortVkey)
	    return false;
      if(sortedBy==1 && dd==doubleSortVkey)
	    return false;
      
      if(di>=0) {
	    sortedBy=0; intSortVkey=di; doubleSortVkey=-1; return true;
      }
      
      if(dd>=0) {
	    sortedBy=1; doubleSortVkey=dd; intSortVkey=-1; return true;
      }
  ...
  static bool sortObjects(const Object* a, const Object* b)
   ...
   int sortedByDouble() const 
    {
     if(sortedBy==1)
       return doubleSortVkey;
  ...
  private:
    ...
    int sortedBy; // -1 for none, 0 for int, 1 for double
...
class ObjectCollection {
  ...
  public:
    ...
    void appendCollectionDeep(ObjectCollection &cc); 	
    ...
    void extendCollection(ObjectCollection &cc, string prefix=""); 
    void extendCollection(ObjectCollection &cc, string fromcolumn, string tocolumn); 
         // extend (i.e. more columns) with a different catalog of the same objects in the same order
    ...
    double sum(int vkey, bool is_integer=false) const;	// sum of column in all objects in the collection
    ...
    double average(int vkey, bool is_integer=false) const;				// average of a column over all objects in the collection
    ...
    double minimum(string colname);					// minimum value of column
    double maximum(string colname);					// maximum value of column
    double sigma68(string colname);                 // half-with between lower 16th and upper 16th percentile
    ...
    void transformColumn(string colname, int (*f)(void));      // set int column according to an int function
    void transformColumn(string colname, int (*f)(int));      // set int column according to an int function
    ...
    int  transformColumnNew(string newcolname, double (*f)(void)); // create new column from no existing one with a function
    ...
    int  transformColumnNew(string newcolname, int (*f)(int), string arg); // create new column from an existing one with a function
    ...
    int  transformColumnNew(string newcolname, int (*f)(double, double), string arg1, string arg2); // create new column from two existing ones with a function
    ...
    int  transformColumnNew(string newcolname, double (*f)(double, double, int), string arg1, string arg2, string arg3); // create new column from three existing ones with a function
    ...
    double tophatDensity(string cx, string cy, double x, double y, double r, bool sortagain=true);       // 2d top-hat kernel density with radius r
    
    double CramerVonMises(string column, ObjectCollection *other, string othercolumn); // two-sided CvM statistic
    double CramerVonMisesRandom(int N, int M); // two-sided CvM statistic
    ...
    int createDoublePropertyIfNecessary(string colname, double init=-1., bool initevenifexists=true); 
    // return column key of present or newly generated double property
    int createIntPropertyIfNecessary(string colname, int init=-1, bool initevenifexists=true);    
    // return column key of present or newly generated int property
    ...
    void sort(string sortcolumn, bool force=false); // sort objects by column (or maybe not sort if force==false and sorted previously)
    ...
    void applyBinAverage(ObjectCollection *bintable, string column, vector<string> binning_columns, string binaveragecolumn_name, string leafcol="");
    
    ObjectCollection* tree(const vector<string> &criteria, const vector<double> &minwidth, int nmin, string target, double badflag);
    void growTree(const vector<string> &criteria, const vector<double> &minwidth, vector<double> min, vector<double> max, int nmin, string target, ObjectCollection* leaves, double badflag);
    void treeAverage(ObjectCollection *tree, vector<string> criteria, string target, string nobjstring="nobj");

    /////////////////////////////////// HELPFUL SMALL STUFF \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
      
    int size() const; 				// return number of objects
    Object* operator[] (const int i) const; 	// return i'th object 
    void print(vector<string> column, ostream& sink=std::cout, bool header=true) const; // print a subset of columns
    void writeToFITS(string filename, vector<string> doubleColumns, vector<string> intColumns=vector<string>(0), 
                                      string extension="OBJECTS") const; // write a subset of columns to FITS file
